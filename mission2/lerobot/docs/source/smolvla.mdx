...
Need to fine-tune on data that lacks language annotations or want to enforce a constant prompt? Pass
`--policy.fixed_language_instruction="Grasp the blue cube."` and the tokenizer will always receive that
instruction regardless of the dataset metadata.
